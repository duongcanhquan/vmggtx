...
export default function AdminLicensesPage() {
  ...
  /** Sau khi tạo cơ sở: hiện link /coso/{slug} để gửi admin cơ sở */
  ...
  async function handleProvision(event: React.FormEvent<HTMLFormElement>) {
    ...
    setToast({
      type: 'success',
      message: `Đã tạo cơ sở. Cổng đăng nhập: ${result.portalPath}`,
    })
    void loadData()
  }
  ...
  return (
    <div className="space-y-6">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {row.slug ? (
                      <a
                        ...
                        className="mt-1 inline-flex items-center gap-1 font-mono text-xs font-semibold text-violet-700 hover:underline"
                      >
                        /coso/{row.slug}
                        <ExternalLink className="h-3 w-3" aria-hidden="true" />
                      </a>
                    ) : (
                      <p className="mt-1 text-xs text-amber-600">Chưa có slug (chạy migration 045)</p>
                    )}
                  </td>
      ...
      {/* Sau khi tạo cơ sở: hiện link cổng /coso/{slug} để gửi admin */}
      {createdPortal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-4 text-sm text-slate-700">
              Gửi cho Quản lý cơ sở (<span className="font-medium">{createdPortal.adminEmail}</span>)
              đường dẫn đăng nhập:
            </p>
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2.5">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setToast({ type: 'success', message: 'Đã sao chép link cổng cơ sở.' })
                }}
            ...
            <p className="mt-2 text-xs text-slate-500">
              Super Admin vẫn đăng nhập tại{' '}
              <span className="font-mono">/login</span> (gốc domain). Cơ sở vào{' '}
              <span className="font-mono">/coso/…</span>
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <a
