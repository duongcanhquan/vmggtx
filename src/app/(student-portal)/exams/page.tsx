...
export default function StudentExamsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await getMyUpcomingStudentExams()
    setRows(res.data)
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 px-4 py-6 sm:px-6">
      <header>
        <div className="mb-2 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-50 text-amber-700">
          <ClipboardCheck className="h-5 w-5" aria-hidden="true" />
        </div>
        <h1 className="font-heading text-2xl font-bold">Lịch thi</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Các kỳ thi sắp tới của lớp bạn đang học (chỉ xem).
        </p>
      </header>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border bg-surface px-6 py-12 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">Chưa có lịch thi sắp tới.</p>
        </div>
      ) : (
        <ul className="space-y-3">
          {rows.map((row) => (
            ...
            >
              ...
              <p className="mt-3 flex items-center gap-1.5 text-sm tabular-nums">
