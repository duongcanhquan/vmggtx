...
- [ ] Lưu ý các migration mới nhất: `042_overview_report.sql` = RPC báo cáo tổng quan dashboard; `043_menu_permissions.sql` = ma trận phân quyền menu động; `044_tenant_licenses.sql` = tầng LICENSE bán account cơ sở (gói module, hạn dùng, giới hạn học viên, chặn đăng nhập khi hết hạn); `045_org_slugs.sql` = slug công khai cơ sở + RPC `get_public_campus_by_slug` cho cổng `/coso/[slug]`.
