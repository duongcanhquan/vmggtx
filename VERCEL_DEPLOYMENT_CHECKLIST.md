...
- [ ] Lưu ý các migration mới nhất: `042`…`045` (slug /coso); `049_user_grants.sql` = quyền kiêm nhiệm; **`050_parent_accounts.sql` = phụ huynh email+mật khẩu** (bắt buộc nếu dùng cổng Gia đình → Phụ huynh); **`051_org_logo.sql` = logo thương hiệu theo org** (cổng login + shell); **`052_crm_admissions_pro.sql` = CRM tuyển sinh**; **`053_crm_lead_profile_ai.sql` = hồ sơ lead đầy đủ (CCCD/PH/sở thích) + cấu hình AI CRM + entity lead**; **`054_crm_counseling_payment.sql` = điểm mạnh/yếu/nhu cầu/tiềm năng + đặt cọc/ghi chú đóng tiền**; **`055_academic_warnings_ops.sql` = cảnh báo học vụ severity + workflow xử lý**; **`056_job_titles.sql` = chức danh + mẫu quyền menu (title ∪ grants)**; **`057_org_holidays.sql` = ngày nghỉ TKB**; **`058_class_schedule_plans.sql` = kế hoạch auto xếp lịch**; **`059_work_tasks.sql` = phân công công việc nội bộ**; **`060_session_facility_id.sql` = gắn CSVC vào buổi học**; **`061_curriculum_subjects.sql` = tín chỉ/tiết/chuẩn đầu ra môn**; **`062_tuition_rules.sql` = công thức học phí + sinh HĐ nháp**; **`063_teacher_subjects.sql` = ngành + môn dạy GV**; **`064_class_groups_teachers.sql` = lớp hành chính + class_teachers**; **`065_lms_rubrics.sql` = rubric LMS + draft/final**; **`067_hr_leave_workdays.sql` = phép/công/lương VP (D28)**; **`068_masv_student_code_sync.sql` = backfill MaSV (QA)**; **`069_perf_crm_and_aggregates.sql` = index CRM + RPC activity stats / sum payments (CRM & dashboard nhanh)**; **`070_facility_room_meta.sql` = meta phòng học**; **`071_campus_admin_view_financials.sql` = Quản lý cơ sở luôn xem lương/đơn giá**; **`072_hr_personnel_dossier.sql` = hồ sơ NS + giấy tờ + probation + nhắc HR**.; **`073_facility_vehicle_type.sql`** = loại xe (vehicle) trong facilities; **`074_payment_atomic_facility_pending.sql`** = thu học phí atomic + đặt CSVC pending/duyệt; **`075_exam_module_publish_pathways.sql; **076_announcement_targeting.sql** = thong bao theo lop/ca nhan`** = module Khảo thí (công bố điểm, phát đề, lộ trình HV). Tùy chọn: `R2_PUBLIC_BASE_URL` nếu dùng CDN công khai cho logo.
